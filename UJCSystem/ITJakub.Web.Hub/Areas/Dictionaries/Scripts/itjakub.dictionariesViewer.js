...
var DictionaryViewer = (function () {
    ...
    DictionaryViewer.prototype.printList = function () {
        ...
        doc.write("</div>");
        doc.close();
        $("link, style").each(function (index, element) {
            $(doc.head).append($(element).clone());
        });
        var css = "body { background-color: white; padding: 0 10px; }" + ".dictionary-result-headword-favorite { display: none; }" + ".dictionary-result-book-list { margin-left: 16px; }";
