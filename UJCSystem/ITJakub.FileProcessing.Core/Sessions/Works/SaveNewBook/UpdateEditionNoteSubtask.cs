...
namespace ITJakub.FileProcessing.Core.Sessions.Works.SaveNewBook
{
    public class UpdateEditionNoteSubtask
    {
        ...
        public long? UpdateEditionNote(long projectId, long bookVersionId, int userId, string comment, BookData bookData)
        {
            ...
            if (dbEditionNoteResource == null)
            {
                ...
                return CreateEditionNoteResource(newDbResource, 1, comment, user, now, bookVersion);
            }
            else
            {
                return CreateEditionNoteResource(dbEditionNoteResource.Resource, dbEditionNoteResource.VersionNumber + 1, comment, user, now, bookVersion);
            }
        }

        private long CreateEditionNoteResource(Resource resource, int version, string comment, User user, DateTime now, BookVersionResource bookVersion)
        {
