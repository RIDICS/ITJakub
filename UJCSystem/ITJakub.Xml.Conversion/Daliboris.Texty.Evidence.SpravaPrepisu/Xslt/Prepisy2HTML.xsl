...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:p="http://www.daliboris.cz/schemata/prepisy.xsd" exclude-result-prefixes="p">
    ...
    <xsl:template match="p:Hlavicka">
                ...
                <xsl:call-template name="VypisHodnotu">
                    <xsl:with-param name="popisek" select="'Datace pramene'" />
    ...
    <xsl:template match="p:Exporty">
        <xsl:if test="child::*">
        <tr class="exporty"><td colspan="2"><b>Exporty</b></td></tr>
            <xsl:apply-templates select="p:Export" />
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="Edice">
        <xsl:if test="p:TitulEdice">
            <tr>
                <td><span class="popisek">Edice</span></td>
                <td>
