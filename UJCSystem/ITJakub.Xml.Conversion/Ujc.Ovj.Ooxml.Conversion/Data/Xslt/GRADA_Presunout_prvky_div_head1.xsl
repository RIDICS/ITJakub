...
<xd:doc>
	...
	<xsl:template match="/">
		<xsl:comment> Presunout_prvky_div_head1 </xsl:comment>
		<xsl:apply-templates />
	</xsl:template>

	<xsl:template match="div[div[head1]]">
		<xsl:copy>
				<xsl:apply-templates select="div[head1]" mode="vynechat" />
				<xsl:apply-templates select="div[position() &gt; 1]" />
		</xsl:copy>
	</xsl:template>
	
	<xsl:template match="div[head1]" mode="vynechat">
		<xsl:apply-templates />
	</xsl:template>
