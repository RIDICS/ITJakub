			...
			<xsl:choose>
				<xsl:when test="count(child::*[(name() = 'head')]) &gt; 0">
					<xsl:variable name="uroven">
						<xsl:value-of select="count(child::*[(name() = 'head')][1]/preceding-sibling::*)+1"/>
					</xsl:variable>
<!--					<xsl:apply-templates select="*[position() &lt; count(child::*[(starts-with(name(), 'head'))][1]/preceding-sibling::*)+1]"/>-->
					<!--<xsl:apply-templates select="*[position() &lt; count(child::*[(starts-with(name(), 'head'))][1]/preceding-sibling::*)+1]"/>-->
