...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xd:doc scope="stylesheet">
		<xd:desc>
			<xd:p><xd:b>Created on:</xd:b> Dec 19, 2014</xd:p>
	...
	<xsl:template match="/">
		<xsl:comment> Seskupit_prvky_explicit+incipit_do_div_2.0 </xsl:comment>
		<xsl:apply-templates />
	</xsl:template>
	...
	<xsl:template match="div[div[@type='explicit']] | div[head[@type='explicit']]">
		<xsl:copy>
			...
			<xsl:for-each-group select="*" group-ending-with="div[@type='explicit'] | head[@type='explicit']">
						<div type='bible'><xsl:apply-templates select="." mode="group"/></div>
					</xsl:for-each-group>
		</xsl:copy>
	</xsl:template>
