...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:ev="http://www.daliboris.cz/schemata/prepisy.xsd" exclude-result-prefixes="xd ev" version="1.0">
	<xsl:template name="catRef">
		...
		<xsl:variable name="output">
			<xsl:if test="contains(ev:Zpracovani/ev:ZpusobVyuziti, 'StaroceskyKorpus')"><xsl:text> #output-text_bank-old_czech</xsl:text></xsl:if>
			<xsl:if test="contains(ev:Zpracovani/ev:ZpusobVyuziti, 'StredoceskyKorpus')"><xsl:text> #output-text_bank-middle_czech</xsl:text></xsl:if>
			<xsl:if test="contains(ev:Zpracovani/ev:ZpusobVyuziti, 'EdicniModul')"><xsl:text> #output-editions</xsl:text></xsl:if>
			<xsl:if test="contains(ev:Zpracovani/ev:ZpusobVyuziti, 'Slovnik')"><xsl:text> #output-dictionary</xsl:text></xsl:if>
			<xsl:if test="contains(ev:Zpracovani/ev:ZpusobVyuziti, 'Mluvnice')"><xsl:text> #output-digitized-grammar</xsl:text></xsl:if>
			<xsl:if test="contains(ev:Zpracovani/ev:ZpusobVyuziti, 'OdbornaLiteratura')"><xsl:text> #output-scholary_literature</xsl:text></xsl:if>
		</xsl:variable>
