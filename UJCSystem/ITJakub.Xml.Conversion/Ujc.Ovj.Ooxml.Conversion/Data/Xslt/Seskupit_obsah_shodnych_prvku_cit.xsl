...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:comment> Seskupit_obsah_shodnych_prvku_cit </xsl:comment>
        <xsl:text xml:space="preserve">
</xsl:text>
        <xsl:apply-templates/>
    </xsl:template>
    ...
    <xsl:template match="tei:cit">
        <xsl:copy>
            ...
            <xsl:for-each-group select="*"
                group-adjacent="if (self::tei:quote) then 0 else position()">
    ...
    <xsl:template match="tei:quote" mode="grouping">
        <xsl:apply-templates />
    </xsl:template>
