...
namespace Ujc.Ovj.Tools.Xml.XsltTransformation
{
	public class XsltTransformationProcess : IXsltTransformationProcess
	{
		...
		public XsltTransformationProcess(IList<IXsltTransformer> xsltTransformers)
			: this()
		{
			XsltTransformers = xsltTransformers;
		}

		public XsltTransformationProcess(string inputFilename, string outputFilename, IList<IXsltTransformer> xsltTransformers)
		...
		public XsltTransformationProcess(string inputFilename, string outputFilename, IList<IXsltTransformer> xsltTransformers, NameValueCollection parameters)
			: this(inputFilename, outputFilename, xsltTransformers)
		{
			MergeXsltTransformationParameters(parameters);
		}

		private void MergeXsltTransformationParameters(NameValueCollection parameters)
		{

			foreach (IXsltTransformer transformer in XsltTransformers)
			{
				if (transformer.Parameters != null && transformer.Parameters.Count > 0)
				{
					foreach (string parameter in transformer.Parameters.AllKeys)
					{
						if (parameters == null || parameters.Count == 0)
						{
							transformer.Parameters.Set(parameter, null);
						}
						else
						{
							transformer.Parameters.Set(parameter, parameters.Get(parameter));
						}
					}
				}
			}
		...
		public void Transform(string inputFilename, string outputFilename, IList<IXsltTransformer> xsltTransformers)
		{
			...
			if (XsltTransformers.Count > 0 && !XsltTransformers[XsltTransformers.Count - 1].XsltInformation.GeneratesResultDocument)
		...
		private void Transform(string inputDirectory, string inputFileMask, string outputDirectory, string outputFilenameExtension)
		{
			...
			foreach (FileInfo fileInfo in fileInfos)
			{
				...
				string subdirectory = GetSubdirecotryPath(fileInfo, inputDirectory);
				if (subdirectory == "\\") subdirectory = string.Empty;

				inputOutputFiles.Add(fileInfo.FullName,
														 Path.Combine(Path.Combine(outputDirectory, subdirectory),
