...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="body/p | body/l">
		<xsl:variable name="name" select="local-name()"/>
		<xsl:variable name="name1" select="'l'"/>
		<xsl:variable name="name2" select="'p'"/>
		
		<!-- Is this the first element in a sequence? -->
		<xsl:if test="local-name(preceding-sibling::*[position()=1]) != $name">
		<!--<xsl:if test="local-name(preceding-sibling::*[position()=1]) != $name1 or local-name(preceding-sibling::*[position()=1]) != $name2">-->
			<xsl:element name="div">
				...
				<!--<xsl:apply-templates select="following-sibling::*[1][local-name()=$name1] | following-sibling::*[1][local-name()=$name2]" mode="next"/>-->
				<!--</xsl:copy>-->
			</xsl:element>
		</xsl:if>
	</xsl:template>
