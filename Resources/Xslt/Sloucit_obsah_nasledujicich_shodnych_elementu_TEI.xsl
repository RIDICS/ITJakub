...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" exclude-result-prefixes="xd" version="1.0">
  ...
  <xsl:strip-space elements="*"/>
	...
	<xsl:template match="/">
		<xsl:comment> Sloucit_obsah_nasledujicich_shodnych_elementu_TEI </xsl:comment>
		<xsl:apply-templates />
	</xsl:template>
  ...
  <!--<xsl:template match="//*[name() = name(following-sibling::*[1])]">-->
	<xsl:template match="p/child::* | l/child::* | head/child::* | thead/child::* | head1/child::* | head2/child::*">
  ...
  <!--<xsl:template match="//*[name() = name(following-sibling::*[1])]" mode="next">-->
	<xsl:template match="p/child::* | l/child::* | head/child::* | thead/child::* | head1/child::* | head2/child::*" mode="next">
		<xsl:variable name="name" select="local-name()"/>
		<xsl:apply-templates />
		<xsl:apply-templates select="following-sibling::*[1][local-name()=$name]" mode="next"/>
	</xsl:template>
