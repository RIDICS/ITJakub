...
<xd:doc>
	...
	<xsl:template match="body">
		<xsl:copy>
			<xsl:choose>
				<xsl:when test="count(/body/child::*[(name() ='head')]) &gt; 0">
					<xsl:apply-templates select="/body/*[position() &lt; count(/body/child::*[(name() ='head')][1]/preceding-sibling::*)+1]" />
					<xsl:apply-templates select="head" mode="walker"/>
					<xsl:apply-templates select="/body/*[position() &gt; count(/body/child::*[(name() ='head')][1]/following-sibling::*)+2]"></xsl:apply-templates>
				</xsl:when>
				<xsl:otherwise>
					<xsl:apply-templates />
				</xsl:otherwise>
...
<!--	<xsl:template match="preceding-sibling::*/head1[1]" mode="zacatek">
	...
	<xsl:template match="head" mode="walker">
	...
	<xsl:template match="*" mode="walker">
		<xsl:apply-templates select="."/>
		<xsl:if test="not(following-sibling::*[1]/self::head)">
