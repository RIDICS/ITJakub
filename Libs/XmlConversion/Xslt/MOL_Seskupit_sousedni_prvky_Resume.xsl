...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:comment> MOL_Seskupit_sousedni_prvky_Resume </xsl:comment>
        <xsl:apply-templates/>
    </xsl:template>
    

    
    <xsl:template match="*[div[@type='resume']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:for-each-group select="*" group-adjacent="exists(self::div[@type='resume'])">
                <xsl:choose>
                    <xsl:when test="current-grouping-key()">
                        <div type='resume'>
                            <!--<xsl:if test="contains(current-group()//*, ' the ')">-->
