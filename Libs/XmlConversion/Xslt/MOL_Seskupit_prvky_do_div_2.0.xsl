...
<xsl:stylesheet exclude-result-prefixes="xs xd" version="2.0" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<xsl:comment> MOL_Seskupit_prvky_do_div_2.0 </xsl:comment>
		<xsl:apply-templates />
	</xsl:template>
		...
		<xsl:copy>
			<xsl:choose>
				<xsl:when test="title[@type='main']">
					<xsl:for-each-group group-starting-with="title[@type='main']" select="*">
	...
	<xsl:template match="title[@type='main']" mode="group">
		<div>
			<title type="main">
