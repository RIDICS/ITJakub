...
declare function vw:getDocumentMetadata($documentId as xs:string)
    as node() {    
    let $collection := collection($vw:collectionPath)
    let $document := $collection//itj:document[tei:teiHeader/tei:fileDesc[@n=$documentId]]   
    return $document[1] (: get first version :)
};

declare function vw:getDocumentVersionMetadata($documentId as xs:string, $documentVersionId as xs:string)
    as node() { 
    let $collection := collection($vw:collectionPath)
    let $document := $collection//itj:document[@versionId=$documentVersionId and tei:teiHeader/tei:fileDesc[@n=$documentId]]   
    return $document 
};