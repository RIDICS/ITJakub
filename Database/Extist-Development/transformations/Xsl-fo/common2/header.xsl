<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xd="http://www.pnp-software.com/XSLTdoc" exclude-result-prefixes="tei xd" version="2.0">
    ...
    <xsl:template name="generateSimpleTitle">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:apply-templates select="ancestor-or-self::tei:TEI/tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:title" mode="simple"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    <xd:doc>
